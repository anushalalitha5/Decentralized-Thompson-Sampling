...
def compute_W_linkfailure(N, type, p, max_iter, args=None):
    ...
    if type == 'complete':
        for t in range(max_iter):
            ...
            flip = np.random.binomial(1,1-p, (N,N)) # 0 denotes link failure
            for i in range(N):
                for j in range(N):
                    A[i][j] = A[i][j]*flip[i][j]
                    A[j][i] = A[j][i]*flip[i][j]
            delta = np.array([[A[i][j] for j in range(N)].count(1) for i in range(N)])
            D = np.diag(delta)
            W_seq[t,:] = np.eye(N, dtype=float) - (D-A)/(delta.max()+1)
    else: 
        raise RuntimeError("Not implemented")
    return W_seq
...
def run_MultiAgent_TS_gossip(max_iter, runs, N, mean_vec, sigma_vec, theta, sigma, exp_num, type_W ='complete', save = True, rerun=False):
    
    rho = N

    filename = './data/TS_N{}_{}_T{}_{}_Run{}_GaussianExp{}'.format(N, type_W, max_iter, 'gossip', runs, exp_num)
    ...
    if not rerun and os.path.isfile(filename):
        ...
        for run in range(runs):
            ...
            W_seq = compute_W_gossip(N, type_W, max_iter)
...
def run_MultiAgent_TS_linkfailure(max_iter, runs, N, mean_vec, sigma_vec, theta, sigma, exp_num, p, type_W ='complete', save = True, rerun=False):
    
    rho = N

    filename = './data/TS_N{}_{}_T{}_{}_{}_Run{}_GaussianExp{}'.format(N, type_W, max_iter, 'linkfailure', p, runs, exp_num)
    ...
    if not rerun and os.path.isfile(filename):
        ...
        for run in range(runs):
            ...
            W_seq = compute_W_linkfailure(N, type_W, p, max_iter)
